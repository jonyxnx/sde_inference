...
# Función para estimar sigma y theta
estimarContinuoCompleto <- function(tasas, tiempos){
  differences <- diff(tasas)  # Calculamos la diferencia de las tasas 
  sum_squared_differences <- sum(differences^2 ) # Sumamos las diferencias al cuadrado
  sigma <- sqrt(sum_squared_differences / tiempos[length(tiempos)]) # Calculamos sigma con variacion total
  ...
  df <- diff(tasas)/(tiempos[3]-tiempos[2]) #Calculamos la derivada de la tasa forward
  theta <- df+(sigma^2)*(tiempos[-c(1)]) # Construimos theta con los valores de las derivadas de la tasa forward
...
SIMULAR <- function(r0, theta_gorro, tiempos, n, trayectorias){
...
SIMULAR_EULER <- function(sigma, T, trayectorias, n){
  ...
  p <- ggplot(df, aes(x = time, y = r, group = factor(rep(1:trayectorias, each = n + 1)))) +
    geom_line(color = 'blue', alpha = 0.5) +
    labs(x = "Tiempo (años)", y = "Tasa de interés", title = "Datos Simulados") +
    theme_minimal()
  print(p)
  return(df)
}
...
set_prueba <- SIMULAR_EULER(.02, 10, 1, 1000)

# Estimar theta y sigma
estimacion <- estimarContinuoCompleto(set_prueba$r, set_prueba$time)
...
for (i in 1:500){
  subset <- head(set_prueba, i+3)
  sigmas[i] <-  estimarContinuoCompleto(subset$r, subset$time)[[2]]
}
...
ggplot(sigmas, aes(x = Iteraciones, y = Sigma)) +
  ...
  geom_hline(yintercept = 0.02, color = "blue", linetype = "dashed", size = 1) +
